...
contract StrategyUSDT3Poolv2 is BaseStrategy {
    ...
    address public crv3;
    string public constant override name = "StrategyUSDT3Poolv2";
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
       ...
       uint256 _wantAvailable = balanceOfWant().sub(_debtOutstanding);
        if (_wantAvailable > 0) {
            ...
            ICurve(threePool).add_liquidity([0,0,_wantAvailable], v.mul(DENOMINATOR.sub(slip)).div(DENOMINATOR));
    ...
    function balanceOfPool(uint256 extra) public view returns (uint256) {
        ...
        return (_balance).mul(ratio).div(1e12);
    }
