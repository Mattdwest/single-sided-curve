...
contract StrategyUSDC3Pool is BaseStrategy {
    ...
    address public crv3;
    string public constant override name = "StrategyUSDC3Pool";
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
       ...
       uint256 _wantAvailable = balanceOfWant().sub(_debtOutstanding);
        if (_wantAvailable > 0) {
            uint256 _availableFunds = IERC20(usdc).balanceOf(address(this));
            uint256 v = _availableFunds.mul(1e30).div(ICurve(threePool).get_virtual_price());
            ICurve(threePool).add_liquidity([0,_availableFunds,0],  v.mul(DENOMINATOR.sub(slip)).div(DENOMINATOR));
    ...
    //this math only deals with want, which is usdc.
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ...
        ICurve(threePool).remove_liquidity_one_coin(threePoolBalance, 1, v);
